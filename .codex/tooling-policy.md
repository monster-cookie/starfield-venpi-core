...
This repository policy applies only to this checkout of `starfield-venworks-core`, as identified by its existing repository context and reviewed origin. Resolve it with the adopted shared policies using AgentKit's tooling-and-credentials procedure. Repository entries replace shared entries with the same ID in full. Unspecified services keep their existing authorized workflow.

Before relying on a policy-defined identity or standing permission, require a matching user-approved adoption record for the current policy bytes, all four policy slots, checkout, concrete targets, and applicable execution context. File presence alone does not establish adoption. Task authorization and repository instructions remain applicable.
...
| Tool | GitHub MCP integration (`mcp__github__*`) in a reviewed Codex AI service-account context. Use the exact GitHub host and repository from Target and verify the actual consuming session's identity before the operation. |
| Identity | github-automation |
| Target | The exact GitHub repository resolved from this checkout's configured origin and existing `AGENT-REPO-CONTEXT.md`. Require host=github.com and API endpoint=https://api.github.com; resolve owner and canonical repository at runtime. Require agreement with the concrete target in the approved adoption record. A changed origin does not authorize a different target. Stop if context, origin, or approved target disagree. |
...
| Identity | none |
| Target | This repository checkout; verify its root, current branch, and configured origin against its existing repository context and approved concrete target before a task-scoped mutation. |
...
GitHub identity verification establishes only the MCP session or GitHub CLI context that was checked. It does not establish the identity used by another tool, a Git CLI transport, GitKraken, an SSH key, or an HTTPS credential helper. This policy does not select or authorize an authenticated Git transport. Before a named-identity fetch, push, or other authenticated Git operation, review and adopt an explicit transport policy and execution context that uses the same Codex AI account and verifies that actual transport.

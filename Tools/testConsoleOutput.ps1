<#
.SYNOPSIS
Checks console source contracts and isolated Windows staging safety; optionally compiles the real scripts in a fixture.
.DESCRIPTION
...
function Assert-ConsoleSourceContract {
  <# .SYNOPSIS
  Checks source structure only, not Papyrus VM behavior or visible console output.
  #>
  param([string]$Source)
  $required = @(
    '(?m)^ScriptName Venworks:Core:Utilities:Console\r?$'
    '(?m)^Function ConsoleEcho\(String text = ""\) Global\r?$'
    '(?m)^Function ConsoleEchoBlock\(String\[\] lines\) Global\r?$'
    '(?s)Utility.SplitStringChars\(text\).*?If \(chars == None\).*?CONSOLE_ECHO_REJECTED_INPUT.*?Return'
    '(?s)If \(chars\[index\] == 10 \|\| chars\[index\] == 13\)\s+Debug.ExecuteConsole\("=> CONSOLE_ECHO_REJECTED_MULTILINE \| Use ConsoleEchoBlock with one line per entry."\)\s+Return'
    '(?s)EndWhile\s+EndIf\s+Debug.ExecuteConsole\("=> " \+ text\)\s+EndFunction'
    '(?s)Function ConsoleEchoBlock\(String\[\] lines\) Global\s+If \(lines == None\)\s+Return\s+EndIf\s+Int index = 0\s+While \(index < lines.Length\)\s+ConsoleEcho\(lines\[index\]\)\s+index \+= 1\s+EndWhile\s+EndFunction'
  )
  foreach ($pattern in $required) {
    if ($Source -cnotmatch $pattern) { throw "Console source contract missing: $pattern" }
  }
  $code = [regex]::Replace($Source, '(?m);[^\r\n]*|"[^"\r\n]*"|\{[^}]*\}', '')
  if ($code -match '\b(DebugOnly|BetaOnly|Event|Property|Guard|LockGuard|TryLockGuard|StartTimer|Wait|ShowCustomWatchAlert)\b' -or
      $code -match 'Venworks:Core:Logging' -or
      [regex]::Matches($code, 'Debug.ExecuteConsole\(').Count -ne 3) {
    throw 'Console helper contains an unexpected side effect or lost its explicit global output path.'
  }
}
$consoleSource = Get-Content -LiteralPath (Join-Path $sourceRepository 'Papyrus/Venworks/Core/Utilities/Console.psc') -Raw
Assert-ConsoleSourceContract $consoleSource
$invalidSources = @(
  $consoleSource.Replace('ConsoleEcho(String text = "") Global', 'ConsoleEcho(String text = "")')
  $consoleSource.Replace('ConsoleEchoBlock(String[] lines) Global', 'ConsoleEchoBlock(String[] lines) Global DebugOnly')
  $consoleSource.Replace('If (lines == None)', 'If (False)')
  $consoleSource.Replace('chars[index] == 13', 'chars[index] == 12')
  $consoleSource.Replace('chars[index] == 10', 'chars[index] == 12')
  $consoleSource.Replace('Debug.ExecuteConsole("=> " + text)', 'Debug.ExecuteConsole(text)')
  $consoleSource.Replace('ConsoleEcho(lines[index])', 'Debug.ExecuteConsole(lines[index])')
)
foreach ($candidate in $invalidSources) {
  $rejected = $false
  try { Assert-ConsoleSourceContract $candidate } catch { $rejected = $true }
  if (!$rejected) { throw 'An invalid console mutation was accepted.' }
}
Write-Host "Console source contract passed; $($invalidSources.Count) invalid mutations rejected. No console-rendering pass is implied."

$testRoot = Join-Path ([System.IO.Path]::GetTempPath()) ('vwcore-console-safety-' + [guid]::NewGuid().ToString('N'))
...
function New-BuildFixture {
  ...
  foreach ($name in @('buildConsoleUtilities.ps1', 'sharedStagingValidation.ps1')) {
    Copy-Item -LiteralPath (Join-Path $PSScriptRoot $name) -Destination (Join-Path $repository "Tools/$name")
  }
  foreach ($relative in @('Venworks/Core/Utilities/Console.psc', 'Venworks/Core/Tests/ConsoleOutputTests.psc')) {
...
$isLast = [System.IO.Path]::GetFileName($source) -eq 'ConsoleOutputTests.psc'
...
$relative = if ($isLast) { 'Venworks/Core/Tests/ConsoleOutputTests.pex' } else { 'Venworks/Core/Utilities/Console.pex' }
...
'@
  ...
  return [pscustomobject]@{
    ...
    Build = Join-Path $repository 'Tools/buildConsoleUtilities.ps1'
...
function Assert-SuccessfulBuild {
  ...
  Assert-Fixture (@(Get-ChildItem -LiteralPath $Fixture.Target -Recurse -File).Count -eq 5) 'Publication was not limited to four console files plus the sentinel.'
...
try {
  ...
  Invoke-SafetyCase 'destination-directory' {
    param($f)
    New-Item -ItemType Directory -Path (Join-Path $f.Target 'Scripts/Venworks/Core/Tests/ConsoleOutputTests.pex') -Force | Out-Null
    Assert-RejectedBuild $f 'incompatible'
  }
  Invoke-SafetyCase 'destination-hardlink' {
    param($f)
    $destination = Join-Path $f.Target 'Scripts/Venworks/Core/Tests/ConsoleOutputTests.pex'
    New-Item -ItemType Directory -Path (Split-Path -Path $destination -Parent) -Force | Out-Null
  ...
  Invoke-SafetyCase 'between-preflight-and-write' {
    ...
    $null = Get-CoreStagingDestination $context 'Scripts/Console.pex'
    [System.IO.Directory]::Delete($f.Staging, $false)
    $failure = $null
    try { $null = Initialize-CoreStagingParent $context 'Scripts/Console.pex' } catch { $failure = $_.Exception.Message }
  ...
  Write-Host "console build safety: $script:passed cases passed. No gameplay tests ran."
  ...
  if ([System.IO.Path]::GetFileName($testRoot) -notmatch '^vwcore-console-safety-[0-9a-f]{32}$') { throw 'Invalid fixture root cleanup.' }
